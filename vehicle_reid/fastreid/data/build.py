...
from .common import CommDataset, JackDataset
...
def _train_loader_from_config(cfg, *, train_set=None, transforms=None, sampler=None, **kwargs):
    ...
    if train_set is None:
        ...
        if cfg.DATASETS.NAMES[0] in ['Jackson','Taipei','Adventure','Flat','Square','Caldot1','Caldot2','Shibuya']:
            train_set = JackDataset(train_items, transforms, relabel=True)
        else:
...
def _test_loader_from_config(cfg, *, dataset_name=None, test_set=None, num_query=0, transforms=None, **kwargs):
    ...
    if test_set is None:
        ...
        if cfg.DATASETS.NAMES[0] in ['Jackson','Taipei','Adventure','Flat','Square','Caldot1','Caldot2','Shibuya']:
            test_set = JackDataset(test_items, transforms, relabel=False)
        else:
...
def fast_batch_collator(batched_inputs):
    ...
    if isinstance(elem, torch.Tensor):
        ...
        return torch.tensor(batched_inputs, dtype=torch.float16)
